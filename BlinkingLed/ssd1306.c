...
/* Includes ------------------------------------------------------------------*/
//#include "LIB_Config.h"  //arek
#include "SSD1306.h"
#include "Fonts.h"
#include "TWI_Master.h"  //arek
...
// I2C arek
static unsigned char TWI_messageBuf[4];
...
static void ssd1306_write_byte(uint8_t chData, uint8_t chCmd) 
{
  // TWI_Master driver based.	
	...
	if (chCmd) {
		TWI_messageBuf[0] = 0x40;
	} else {
        TWI_messageBuf[0] = 0x00;			
	}
	TWI_messageBuf[0] = chData;
    ...
    for (i = 0; i < chSize; i ++) {   
		if (chSize == 12) {
			if (chMode) {
				...
				chTemp = pgm_read_byte(&(c_chFont1206[chChr][i]));  // atmel  
			} else {
				//chTemp = ~c_chFont1206[chChr][i];
				chTemp = ~(pgm_read_byte(&(c_chFont1206[chChr][i])));  // atmel
			}
			...
			if (chMode) {
				...
				chTemp = pgm_read_byte(&(c_chFont1608[chChr][i]));  // atmel
			} else {
				//chTemp = ~c_chFont1608[chChr][i];
				chTemp = ~(pgm_read_byte(&(c_chFont1608[chChr][i])));  // ateml
			}
		}
...
void ssd1306_draw_1616char(uint8_t chXpos, uint8_t chYpos, uint8_t chChar)
{
	...
	for (i = 0; i < 32; i ++) {
		...
		chTemp = pgm_read_byte(&(c_chFont1612[chChar - 0x30][i]));  //atmel
		for (j = 0; j < 8; j ++) {
...
void ssd1306_draw_3216char(uint8_t chXpos, uint8_t chYpos, uint8_t chChar)
{
	...
	for (i = 0; i < 64; i ++) {
		...
		chTemp = pgm_read_byte(&(c_chFont3216[chChar - 0x30][i]));  // atmel
		for (j = 0; j < 8; j ++) {
			chMode = chTemp & 0x80? 1 : 0; 
			ssd1306_draw_point(chXpos, chYpos, chMode);
			chTemp <<= 1;
